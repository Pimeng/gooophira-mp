...
本文档记录 `gooophira-mp-server` 当前已经实现的 HTTP API。HTTP 服务的实际 API 前缀为 `/api`；下文为简洁起见省略此前缀。例如，文档中的 `GET /room` 实际请求地址是 `GET /api/room`。
...
Docker 镜像默认开启 HTTP 服务。未配置永久 `ADMIN_TOKEN` 时，管理员接口只能通过 OTP/CLI 提权获取临时 token。HTTP 服务由 `internal/server/api/httpapi` 提供；统一路由、CORS、限流和错误分发位于 `internal/server/api/httpapi/server.go`。
...
- 当前路由实际使用 `GET`、`POST`、`DELETE` 和 `OPTIONS`；CORS 预检响应目前声明的允许方法为 `GET`、`POST`、`OPTIONS`。
- 普通 HTTP 请求按客户端 IP 限流；超过限制返回 `429`、`error: rate-limited` 和 `Retry-After` 响应头。
- `/api/ws` 是长连接，不计入普通 HTTP 限流。
- 未匹配路由返回 `404`。
...
上传由 Agent 处理，Server 只负责验证 Phira 用户 token、拼接回放 ID 并通过 IPC 转发。Agent 不可用返回 `503 agent-unavailable`，其他代理错误通常返回 `502`。当前实现不会由 Server 直接读取或上传回放文件。
...
POST 接受可热更新配置项的 JSON patch。成功返回 `updatedKeys`、当前 `config` 和 `rollbackAvailable`；GET 还返回 `managedKeys`。非法键、启动期配置或不支持的键返回 `400 bad-runtime-config`，响应还会列出 `invalidKeys`、`startupOnlyKeys`、`unsupportedKeys` 和 `managedKeys`。没有回滚快照时返回 `409 runtime-config-rollback-unavailable`。

当前可热更新的配置键为：

```text
ROOM_CREATION_ENABLED, REPLAY_ENABLED, ROOM_MAX_USERS,
PLAYING_RECONNECT_GRACE, MAX_ROOMS, MAX_CONNECTIONS,
CONNECTION_RATE_LIMIT, COMMAND_RATE_LIMIT,
HTTP_RATE_LIMIT_MAX_REQUESTS, HTTP_RATE_LIMIT_WINDOW_MS,
CHAT_ENABLED, REPLAY_TTL_DAYS, ROOM_LIST_TIP, LOG_LEVEL,
LOG_COMPRESS_AFTER_DAYS, LOG_MAX_TOTAL_MB
```
...
日志 `limit` 默认为 `200`，最大为 `500`。命令最大长度为 500。命令为空返回 `400 bad-command`，命令过长返回 `400 command-too-long`，控制台未准备好返回 `503 console-not-ready`，命令执行失败返回 `500 command-failed`。
...
返回 `ssid`、`expiresIn` 和 `mode`，会话默认 1 分钟有效。OTP 验证码打印在服务端终端，不通过 HTTP 返回。配置永久 `ADMIN_TOKEN` 时该功能返回 `403 otp-disabled-when-token-configured`。
...
CLI 审批等待时返回 `202 pending-approval`；拒绝返回 `403 approval-denied`；审批会话过期或无效返回 `401 invalid-or-expired-session`，IP 不匹配返回 `403 ip-mismatch`；成功返回临时管理员 token 及过期时间。OTP 错误或过期返回 `401 invalid-or-expired-otp`，连续失败达到 3 次后 IP/会话会被封禁 10 分钟。
...
- HTTP 路由：`internal/server/api/httpapi/feishu_registration.go`
- 管理路由接入：`internal/server/api/httpapi/admin.go`
- Agent 注册任务：`internal/agent/feishu/manager.go`
- IPC 方法和请求响应结构：`internal/common/agentproto/protocol.go`
...
- HTTP 统一路由：`internal/server/api/httpapi/server.go`
- 管理员鉴权：`internal/server/api/httpapi/admin.go`
- 飞书注册接口：`internal/server/api/httpapi/feishu_registration.go`
- 飞书注册任务：`internal/agent/feishu/manager.go`
- 回放：`internal/server/api/httpapi/replay.go`
- 统计：`internal/server/api/httpapi/stats_handler.go`
- WebSocket：`internal/server/api/httpapi/ws.go`
- Agent IPC：`internal/agent/ipc/`、`internal/common/agentproto/`
