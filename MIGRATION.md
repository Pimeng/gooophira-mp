...
> 多实例共享）**/**入口层（CLI 参数全集 + 本地化校验 / 启动日志本地化 / 配置文件首次自动生成）**
> 全部就位并测试覆盖。**308 测试**全绿（16 包），真机已验。原 tphira-mp 源码功能链路 100% 迁移。
...
- [x] HTTP/WS 服务启动 / configWatcher 热重载 / CLI 控制台 / 完整 startServer 编排（均已在 main 装配，见 Stage 4/5/9）。
...
**模块健康**：`go build ./...`、`go vet ./...`、`go test ./... -count=2` 全部通过（**308 测试，16 包**，gofmt 干净）。
**运行**：`go run ./cmd/server -c server_config.example.yml`（`-c` = `-config` 别名；认证需可达 Phira API）；
常用 CLI 参数 `-host`/`-port`(`-p`)/`-http-service`/`-http-port`/`-room-max-users`/`-server-name`/`-monitors`/`-gui`
（优先级高于环境变量与配置文件，非法值按语言本地化报错）；首次运行未带配置文件时自动生成 `server_config.yml`；
加 `--gui` 或配置 `GUI: true` 弹出控制台窗口；配置 `REDIS.ENABLED: true` 启用多实例共享缓存。
...
### Stage 9 — 入口层对齐收尾（CLI 参数 / 启动日志 / 配置自动生成）  ✅ 完成
源: `main.ts`（CLI 参数解析 + requireParse 本地化校验）、`core/server.ts`（autoCreateConfig /
ensureDefaultConfigFile）、`core/configValues.ts`（parse* 校验规则）。复盘发现入口层尚有未迁移项
（`cli-invalid-*` 文案已 embed 但无人调用、启动期若干日志硬编码、首次运行不生成配置文件），本阶段补齐。
- [x] **启动期日志全本地化**：`cmd/server/main.go` 原先硬编码的 11 条启动/配置/Redis/监听/热重载
      日志（如 `config file not found...`）改走 `l10n.TL`，新增 `log-locale-overrides-loaded` /
      `log-config-loaded` / `log-config-not-found` / `log-redis-fallback` / `log-redis-enabled` /
      `log-admin-data-load-failed` / `log-listen-failed` / `log-http-start-failed` /
      `log-config-reload-skipped` / `log-config-reloaded` / `log-config-reload-restart` 键（6 语言齐备）。
      启动期语言在 `LoadMerged` 后即解析（与 `state.ServerLang` 同源），供早期日志使用。
- [x] **CLI 参数完整对齐 TS**：`main.go` 补 `-host` / `-port`(`-p`) / `-http-service` / `-http-port` /
      `-room-max-users` / `-server-name` / `-monitors` / `-gui` 及 `-config`(`-c`) 别名。校验类参数用
      字符串接收，经 `applyCLIOverrides` + `config.Parse*`（导出的 `cliparse.go` 包装 env/文件同款解析器）
      校验，非法时按启动期语言输出 `cli-invalid-*` 本地化错误并退出（对齐 TS `requireParse`）。优先级
      命令行 > 环境变量 > 文件 > 默认。真机验证 zh/en 双语错误与有效覆盖均生效。
- [x] **配置文件首次自动生成**：`config/autocreate.go` `EnsureDefaultFile`（缺失时优先复制同目录
      `server_config.example.yml`，否则写内置最小模板 `DefaultConfigYAML`；已存在不覆盖），main 在
      `LoadMerged` 前调用，当次即加载生效（对齐 TS `autoCreateConfig`）。新增 `log-config-created` 键。
      `autocreate_test.go` 3 测。**未迁移在线拉取档**：TS 的 GitHub/CNB release 在线拉取示例 + locales.json
      在 Go 不适用——其 URL 指向上游 tphira 发布镜像，且 Go 已 `go:embed` 全部 6 语言（无需在线补齐），
      故刻意不引入启动期出站网络请求。

**迁移完成**：原 tphira-mp 源码的服务端 + GUI + 缓存 + 入口层（CLI/启动日志/配置自动生成）功能链路已
100% 迁移，无剩余待办。
