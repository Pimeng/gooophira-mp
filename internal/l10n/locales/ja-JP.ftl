...
chat-replay-recorder-hint = ℹ️ ルームリスト/入退室情報に `「{ $name }」` という名前のプレイヤーがいることに気付くかもしれません。このユーザーは本対局のリプレイを録画するためのものです（クライアント動作のトリガーのみ）。このプレイヤーはゲームに参加せず、対局結果にも影響しません。通常通りプレイし、入退室通知は無視してください。
chat-late-join-hint = ℹ️ 対局進行中に参加しました。本対局はすでに開始されているため途中で挿入できず、本局は自動的に放棄扱いとなります——これは異常ではなく、操作も不要で、あなたのスコアや他のプレイヤーの対局にも影響しません。本局が終了するまで部屋で待機してください。次局の開始後、通常通り参加できます。
...
log-replay-fake-monitor-id = リプレイ用仮想観戦者のユーザー ID を { $id } に設定しました。クライアントはこの ID で実際のアバター/ニックネームを取得します
