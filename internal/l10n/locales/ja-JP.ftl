...
log-agent-outbox-unavailable = Agent outbox を利用できません：{ $error }
log-agent-outbox-shutdown-failed = Agent outbox の終了に失敗しました：{ $error }
log-agent-ipc-unavailable = Agent IPC を利用できません：{ $error }
log-agent-ipc-listen = Agent IPC は { $addr } で待機中
log-agent-ipc-shutdown-failed = Agent IPC の終了に失敗しました：{ $error }
log-agent-stopped = Agent が停止しました：{ $error }
log-agent-config-not-found = Agent 設定ファイル { $path } が見つからないため、空の設定を使用します
log-agent-load-config-failed = Agent 設定の読み込みに失敗しました：{ $error }
log-agent-open-inbox-failed = Agent inbox のオープンに失敗しました：{ $error }
log-agent-open-webhook-ledger-failed = Agent Webhook 台帳のオープンに失敗しました：{ $error }
log-agent-open-webhook-processor-failed = Agent Webhook プロセッサーのオープンに失敗しました：{ $error }
log-agent-open-stats-failed = Agent 統計データベースのオープンに失敗しました：{ $error }
log-agent-open-stats-processor-failed = Agent 統計プロセッサーのオープンに失敗しました：{ $error }
log-agent-stats-cleanup-failed = Agent 統計データのクリーンアップに失敗しました：{ $error }
log-agent-stats-vacuum-failed = Agent 統計データベースの整理に失敗しました：{ $error }
log-agent-open-upload-failed = Agent リプレイアップロードストアのオープンに失敗しました：{ $error }
log-agent-open-upload-processor-failed = Agent リプレイアップロードプロセッサーのオープンに失敗しました：{ $error }
log-agent-legacy-config-flag = `-webhook-config` は非推奨です。`-config` を使用してください
log-agent-legacy-config-file = 設定ファイル { $path } は非推奨です。{ $target } に移行してください
log-agent-discovery-unavailable = Agent はサーバーのディスカバリーファイルを読み取れません：{ $error }
log-agent-connected = サーバー { $server } に接続しました（Agent プロトコル v{ $version }）
log-agent-server-connection-lost = Agent とサーバーの接続が切断されました：{ $error }
log-pprof-start-failed = pprof 診断サービスの起動に失敗しました：{ $error }
log-pprof-listen = pprof 診断サービスは { $addr } で待機中
log-pprof-shutdown-failed = pprof 診断サービスの終了に失敗しました：{ $error }
...
log-webhook-gave-up = Webhookはイベント { $event } の { $type } { $url } への配信を断念しました
log-webhook-request-error = Webhookリクエストエラー：{ $error }
log-webhook-bad-status = Webhookの { $url } への配信がステータス { $status } を返しました（再試行可能：{ $retryable }）
