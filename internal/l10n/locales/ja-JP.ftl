...
chat-replay-recorder-hint = ルームリスト/入退室情報に `「{ $name }」` という名前のプレイヤーがいることに気付くかもしれません。このユーザーは本対局のリプレイを録画するためのものです（クライアント動作のトリガーのみ）。このプレイヤーはゲームに参加せず、対局結果にも影響しません。通常通りプレイし、入退室通知は無視してください。
