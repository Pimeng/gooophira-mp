...
chat-play-deadline = { $seconds }초 정산 시한이 초과되었습니다. 미정산 플레이어는 자동으로 중단되었으며, 이번 라운드가 종료되었습니다
...
log-agent-outbox-unavailable = Agent outbox를 사용할 수 없습니다: { $error }
log-agent-outbox-shutdown-failed = Agent outbox 종료에 실패했습니다: { $error }
log-agent-ipc-unavailable = Agent IPC를 사용할 수 없습니다: { $error }
log-agent-ipc-listen = Agent IPC가 { $addr }에서 수신 대기 중
log-agent-ipc-shutdown-failed = Agent IPC 종료에 실패했습니다: { $error }
log-agent-stopped = Agent가 중지되었습니다: { $error }
log-agent-config-not-found = Agent 구성 파일 { $path }을(를) 찾을 수 없어 빈 구성을 사용합니다
log-agent-load-config-failed = Agent 구성 로드에 실패했습니다: { $error }
log-agent-open-inbox-failed = Agent inbox 열기에 실패했습니다: { $error }
log-agent-open-webhook-ledger-failed = Agent Webhook 원장 열기에 실패했습니다: { $error }
log-agent-open-webhook-processor-failed = Agent Webhook 프로세서 열기에 실패했습니다: { $error }
log-agent-open-stats-failed = Agent 통계 데이터베이스 열기에 실패했습니다: { $error }
log-agent-open-stats-processor-failed = Agent 통계 프로세서 열기에 실패했습니다: { $error }
log-agent-stats-cleanup-failed = Agent 통계 정리에 실패했습니다: { $error }
log-agent-stats-vacuum-failed = Agent 통계 데이터베이스 정리에 실패했습니다: { $error }
log-agent-open-upload-failed = Agent 리플레이 업로드 저장소 열기에 실패했습니다: { $error }
log-agent-open-upload-processor-failed = Agent 리플레이 업로드 프로세서 열기에 실패했습니다: { $error }
log-agent-legacy-config-flag = `-webhook-config`은(는) 더 이상 사용되지 않습니다. `-config`를 사용하세요
log-agent-legacy-config-file = 구성 파일 { $path }은(는) 더 이상 사용되지 않습니다. { $target }(으)로 이동하세요
log-agent-discovery-unavailable = Agent가 서버 discovery 파일을 읽을 수 없습니다: { $error }
log-agent-connected = 서버 { $server }에 연결되었습니다(Agent 프로토콜 v{ $version })
log-agent-server-connection-lost = Agent와 서버의 연결이 끊겼습니다: { $error }
log-pprof-start-failed = pprof 진단 서비스 시작에 실패했습니다: { $error }
log-pprof-listen = pprof 진단 서비스가 { $addr }에서 수신 대기 중
log-pprof-shutdown-failed = pprof 진단 서비스 종료에 실패했습니다: { $error }
