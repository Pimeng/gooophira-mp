...
chat-replay-recorder-hint = ℹ️ 방 목록/입퇴장 정보에 `「{ $name }」`이라는 이름의 플레이어가 있는 것을 알아차리실 수 있습니다. 이 사용자는 이 경기의 리플레이를 녹화하기 위한 것입니다(클라이언트 동작만 트리거). 이 플레이어는 게임에 참여하지 않으며 경기 결과에도 영향을 주지 않습니다. 평소처럼 플레이하시고, 입퇴장 알림은 무시하셔도 됩니다.
chat-late-join-hint = ℹ️ 경기 진행 중에 참여하셨습니다. 이번 경기는 이미 시작되어 도중에 삽입할 수 없으므로, 이번 경기는 자동으로 포기 처리됩니다——이는 오류가 아니며 조치할 필요도 없고, 점수나 다른 플레이어의 경기에도 영향을 주지 않습니다. 이번 경기가 끝날 때까지 방에서 대기해 주세요. 다음 경기가 시작되면 정상적으로 참여하실 수 있습니다.
...
log-replay-fake-monitor-id = 리플레이 가상 관전자 사용자 ID를 { $id }(으)로 설정했습니다. 클라이언트는 이 ID로 실제 아바타/닉네임을 가져옵니다
