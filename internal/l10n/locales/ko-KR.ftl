...
chat-replay-recorder-hint = 방 목록/입퇴장 정보에 `{ $name }`이라는 이름의 플레이어가 있는 것을 알아차리실 수 있습니다. 이 사용자는 이 경기의 리플레이를 녹화하기 위한 것입니다(클라이언트 동작만 트리거). 이 플레이어는 게임에 참여하지 않으며 경기 결과에도 영향을 주지 않습니다. 평소처럼 플레이하시고, 입퇴장 알림은 무시하셔도 됩니다.
