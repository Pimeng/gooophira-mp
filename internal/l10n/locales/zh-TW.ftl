...
log-agent-outbox-unavailable = Agent outbox 無法使用：{ $error }
log-agent-outbox-shutdown-failed = Agent outbox 關閉失敗：{ $error }
log-agent-ipc-unavailable = Agent IPC 無法使用：{ $error }
log-agent-ipc-listen = Agent IPC 監聽於 { $addr }
log-agent-ipc-shutdown-failed = Agent IPC 關閉失敗：{ $error }
log-agent-stopped = Agent 已停止：{ $error }
log-agent-config-not-found = 找不到 Agent 設定檔 { $path }，使用空設定
log-agent-load-config-failed = 載入 Agent 設定失敗：{ $error }
log-agent-open-inbox-failed = 開啟 Agent inbox 失敗：{ $error }
log-agent-open-webhook-ledger-failed = 開啟 Agent Webhook 帳本失敗：{ $error }
log-agent-open-webhook-processor-failed = 開啟 Agent Webhook 處理器失敗：{ $error }
log-agent-open-stats-failed = 開啟 Agent 統計資料庫失敗：{ $error }
log-agent-open-stats-processor-failed = 開啟 Agent 統計處理器失敗：{ $error }
log-agent-stats-cleanup-failed = Agent 統計資料清理失敗：{ $error }
log-agent-stats-vacuum-failed = Agent 統計資料庫整理失敗：{ $error }
log-agent-open-upload-failed = 開啟 Agent 回放上傳儲存失敗：{ $error }
log-agent-open-upload-processor-failed = 開啟 Agent 回放上傳處理器失敗：{ $error }
log-agent-legacy-config-flag = `-webhook-config` 已棄用，請改用 `-config`
log-agent-legacy-config-file = 設定檔 { $path } 已棄用，請遷移至 { $target }
log-agent-discovery-unavailable = Agent 無法讀取伺服器探索檔案：{ $error }
log-agent-connected = 已連線至伺服器 { $server }，Agent 協定版本 { $version }
log-agent-server-connection-lost = Agent 與伺服器的連線已中斷：{ $error }
log-pprof-start-failed = pprof 診斷服務啟動失敗：{ $error }
log-pprof-listen = pprof 診斷服務監聽於 { $addr }
log-pprof-shutdown-failed = pprof 診斷服務關閉失敗：{ $error }
...
log-feishu-send-failed = 飛書傳送失敗（logId={ $logId }，code={ $code }）：{ $msg }
log-webhook-queue-dropped = Webhook 佇列已滿，丟棄事件：{ $event }
log-webhook-rejected = Webhook 目標拒絕事件 { $event }（{ $type } { $url }），不再重試
log-webhook-gave-up = Webhook 放棄投遞事件 { $event } 到 { $type } { $url }
log-webhook-request-error = Webhook 請求錯誤：{ $error }
log-webhook-bad-status = Webhook 投遞 { $url } 回傳狀態碼 { $status }（可重試：{ $retryable }）
