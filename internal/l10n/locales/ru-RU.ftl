...
chat-replay-recorder-hint = Вы можете заметить в списке комнат/уведомлениях о входе-выходе игрока с именем `{ $name }`; этот пользователь используется для записи повторов этой партии (только инициирует поведение клиента). Этот игрок не участвует в игре и не влияет на результаты. Играйте как обычно и игнорируйте его уведомления о входе/выходе.
