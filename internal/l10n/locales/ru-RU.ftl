...
chat-replay-recorder-hint = ℹ️ Вы можете заметить в списке комнат/уведомлениях о входе-выходе игрока с именем `「{ $name }」`; этот пользователь используется для записи повторов этой партии (только инициирует поведение клиента). Этот игрок не участвует в игре и не влияет на результаты. Играйте как обычно и игнорируйте его уведомления о входе/выходе.
chat-late-join-hint = ℹ️ Вы присоединились во время партии. Поскольку эта партия уже началась, вас нельзя вставить в неё, поэтому этот раунд будет автоматически засчитан как покинутый — это не ошибка, никаких действий не требуется, и это не повлияет на ваш счёт или партии других игроков. Пожалуйста, подождите в комнате окончания этой партии; в следующем раунде вы сможете участвовать как обычно.
...
log-replay-fake-monitor-id = ID пользователя виртуального наблюдателя повтора установлен в { $id }; клиент получит реальный аватар/ник по этому ID
