...
system-user-name = 系统
chat-replay-recorder-hint = 你好，我是本服的Bot，协助你在本服务器顺利联机，祝游戏愉快！
chat-late-join-hint = 你在对局进行中加入房间。由于本局已开始，无法将你中途插入，因此本局将自动计为已放弃——这并非异常，也无需任何操作，不会影响你的分数或他人的对局。请耐心在房间内等待本局结束，下一局开始后你即可正常参与。
...
chat-game-ranking-title = 本轮排名
chat-game-ranking-line = { $rank }. { $name } - 分数：{ $score }，准度：{ $acc }%{ $hasStd ->
    [true] ，误差：±{ $std }ms
    *[false] {""}
    }
...
chat-maintenance-enabled = 服务器已进入维护模式，暂停新玩家加入，请尽快结束当前对局
chat-maintenance-disabled = 服务器维护已结束，恢复正常
chat-server-stopping = 服务器即将关闭维护，对局结束后连接将断开
...
chat-record-send-template =
    ...
    { $user }[{$userid}] 结算详情：
        分数：{ $score }，准度：{ $acc }%{ $hasStd ->
        [true] ，误差：±{ $std }ms，无暇度分数：{ $stdScore }
        *[false] {""}
        }{ $isAp ->
        [true] ，AP！！！
        *[false] { $fc -> [true] ，全连 *[false] {""} }
        }
...
# 成绩 mod 显示名（位序见 recordChatMods）
chat-record-mod-autoplay = 自动游玩
chat-record-mod-flip-x = X轴翻转
chat-record-mod-hide-top = 上隐
chat-record-mod-hide-bottom = 下隐
chat-record-mod-club = 夜店
chat-record-mod-rainbow = 彩虹
chat-record-mod-no-shader = 无着色器
...
log-agent-outbox-unavailable = Agent outbox 不可用：{ $error }
log-agent-outbox-shutdown-failed = Agent outbox 关闭失败：{ $error }
log-agent-ipc-unavailable = Agent IPC 不可用：{ $error }
log-agent-ipc-listen = Agent IPC 监听于 { $addr }
log-agent-ipc-shutdown-failed = Agent IPC 关闭失败：{ $error }
log-agent-stopped = Agent 已停止：{ $error }
log-agent-config-not-found = 未找到 Agent 配置文件 { $path }，使用空配置
log-agent-load-config-failed = 加载 Agent 配置失败：{ $error }
log-agent-open-inbox-failed = 打开 Agent inbox 失败：{ $error }
log-agent-open-webhook-ledger-failed = 打开 Agent Webhook 账本失败：{ $error }
log-agent-open-webhook-processor-failed = 打开 Agent Webhook 处理器失败：{ $error }
log-agent-open-stats-failed = 打开 Agent 统计数据库失败：{ $error }
log-agent-open-stats-processor-failed = 打开 Agent 统计处理器失败：{ $error }
log-agent-stats-cleanup-failed = Agent 统计数据清理失败：{ $error }
log-agent-stats-vacuum-failed = Agent 统计数据库整理失败：{ $error }
log-agent-open-upload-failed = 打开 Agent 回放上传存储失败：{ $error }
log-agent-open-upload-processor-failed = 打开 Agent 回放上传处理器失败：{ $error }
log-agent-legacy-config-flag = `-webhook-config` 已弃用，请改用 `-config`
log-agent-legacy-config-file = 配置文件 { $path } 已弃用，请迁移到 { $target }
log-agent-discovery-unavailable = Agent 无法读取服务端发现文件：{ $error }
log-agent-connected = 已连接服务端 { $server }，Agent 协议版本 { $version }
log-agent-server-connection-lost = Agent 与服务端的连接已断开：{ $error }
log-pprof-start-failed = pprof 诊断服务启动失败：{ $error }
log-pprof-listen = pprof 诊断服务监听于 { $addr }
log-pprof-shutdown-failed = pprof 诊断服务关闭失败：{ $error }
...
log-room-host-changed-admin = 房间 “{ $room }” 房主变更（管理员）：{ $old } -> { $next }
...
cli-help =
    ...
    nexthost <roomId> <userId>    - 指定房间下一轮房主（仅循环模式生效）
    lock <roomId> <on|off>          - 强制锁定/解锁房间
    cycle <roomId> <on|off>         - 开关房间循环模式
    sethost <roomId> <userId>       - 立即转移房主
    roominfo <roomId>               - 查看房间详情
    disband <roomId>              - 解散房间
...
cli-bad-max-users = 无效的人数（1-32767）
cli-room-max-users-set = 已设置房间 { $room } 最大人数为 { $count }
cli-room-disbanded = 已解散房间 { $room }

cli-nexthost-set = 已指定房间 { $room } 下一轮房主为用户 { $userId }（仅循环模式下生效）
cli-nexthost-not-cycle = 房间 { $room } 未启用循环模式，无法指定下一轮房主
cli-nexthost-user-not-in-room = 用户 { $userId } 不在房间 { $room } 内

cli-admin-actor = 管理员
cli-bad-toggle = 无效的开关参数（应为 on 或 off）
...
cli-room-locked = 已锁定房间 { $room }
cli-room-unlocked = 已解锁房间 { $room }
cli-room-cycle-on = 已开启房间 { $room } 循环模式
cli-room-cycle-off = 已关闭房间 { $room } 循环模式
cli-sethost-set = 已转移房间 { $room } 房主为用户 { $userId }
cli-sethost-already-host = 用户 { $userId } 已是房间 { $room } 的房主
cli-sethost-user-not-in-room = 用户 { $userId } 不在房间 { $room } 内
cli-roominfo-header = 房间 { $room } 信息
cli-roominfo-line1 = 状态：{ $state } | 房主：{ $host } | 最大人数：{ $maxUsers }
cli-roominfo-line2 = 锁定：{ $locked } | 循环：{ $cycle } | 比赛：{ $contest }
cli-roominfo-line3 = 谱面：{ $chart }
cli-roominfo-players = 玩家（{ $count }）：{ $list }
cli-roominfo-monitors = 观战者（{ $count }）：{ $list }
