...
chat-play-deadline = Превышен { $seconds }-секундный лимит расчёта. Нерасчитанные игроки были автоматически прерваны. Этот раунд окончен.
...
log-agent-outbox-unavailable = Хранилище outbox Agent недоступно: { $error }
log-agent-outbox-shutdown-failed = Не удалось завершить работу outbox Agent: { $error }
log-agent-ipc-unavailable = IPC Agent недоступен: { $error }
log-agent-ipc-listen = IPC Agent прослушивает адрес { $addr }
log-agent-ipc-shutdown-failed = Не удалось завершить работу IPC Agent: { $error }
log-agent-stopped = Agent остановлен: { $error }
log-agent-config-not-found = Файл конфигурации Agent не найден: { $path }; используется пустая конфигурация
log-agent-load-config-failed = Не удалось загрузить конфигурацию Agent: { $error }
log-agent-open-inbox-failed = Не удалось открыть inbox Agent: { $error }
log-agent-open-webhook-ledger-failed = Не удалось открыть журнал Webhook Agent: { $error }
log-agent-open-webhook-processor-failed = Не удалось открыть обработчик Webhook Agent: { $error }
log-agent-open-stats-failed = Не удалось открыть базу статистики Agent: { $error }
log-agent-open-stats-processor-failed = Не удалось открыть обработчик статистики Agent: { $error }
log-agent-stats-cleanup-failed = Не удалось очистить статистику Agent: { $error }
log-agent-stats-vacuum-failed = Не удалось выполнить очистку базы статистики Agent: { $error }
log-agent-open-upload-failed = Не удалось открыть хранилище загрузок реплеев Agent: { $error }
log-agent-open-upload-processor-failed = Не удалось открыть обработчик загрузок реплеев Agent: { $error }
log-agent-legacy-config-flag = `-webhook-config` устарел; используйте `-config`
log-agent-legacy-config-file = Файл конфигурации { $path } устарел; перенесите его в { $target }
log-agent-discovery-unavailable = Agent не может прочитать файл обнаружения сервера: { $error }
log-agent-connected = Подключено к серверу { $server }, протокол Agent v{ $version }
log-agent-server-connection-lost = Соединение Agent с сервером потеряно: { $error }
log-pprof-start-failed = Не удалось запустить диагностику pprof: { $error }
log-pprof-listen = Диагностика pprof прослушивает адрес { $addr }
log-pprof-shutdown-failed = Не удалось завершить диагностику pprof: { $error }
...
log-webhook-gave-up = Webhook прекратил доставку события { $event } на { $type } { $url }
log-webhook-request-error = Ошибка запроса Webhook: { $error }
log-webhook-bad-status = Доставка Webhook на { $url } вернула статус { $status } (повтор: { $retryable })
